...
class TrikiOnline:
    ...
    def get_state(self, pid):
        return {"board": self.board, "turn": self.current_turn, "role": self.players.get(pid), "players_count": len(self.players), "game_over": self.game_over, "winner": self.winner, "scores": self.scores}
...
class Minesweeper:
    ...
    def reveal(self, r, c):
        ...
        if sum(row.count("hidden") for row in self.visible) == 15: self.game_over, self.winner = True, True
        return True
    ...
    def get_state(self):
        return {"visible": self.visible, "values": [[self.board[r][c] if self.visible[r][c] == "revealed" else "" for c in range(10)] for r in range(10)], "game_over": self.game_over, "winner": self.winner, "rows": 10, "cols": 10}
...
def triki_join():
    ...
    if rid not in games: return jsonify({"error": "No"}), 404
...
def ms_new():
    gid = str(uuid.uuid4()); games[gid] = Minesweeper()
...
@app.route('/')
def root(): return send_from_directory(FRONTEND_DIR, 'index.html')
